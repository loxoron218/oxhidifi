...
/// Synchronizes the `dr_is_best` status in the database with the JSON store.
...
pub async fn synchronize_dr_is_best_from_store(pool: &SqlitePool) -> Result<()> {
    ...
    // Fetch all albums from the database to ensure we can update all their dr_is_best statuses
    let all_albums = query("SELECT id, title, artist_id, folder_id, dr_is_best FROM albums")
        .fetch_all(pool)
        .await?;

    // Process each album to synchronize dr_is_best status
    for album_row in all_albums {
        ...
        let current_dr_is_best: bool = album_row.get("dr_is_best");
        ...
        // Determine if the album should be marked as DR best based on the DrValueStore
        let should_be_best = dr_store.contains(&album_key);

        // Skip to next album if status is already correct
        if should_be_best == current_dr_is_best {
            continue;
        }
...
/// Synchronizes the `dr_is_best` status in the database with the JSON store in the background.
/// This function runs asynchronously and can optionally report progress through a callback.
pub async fn synchronize_dr_is_best_background(
    pool: Arc<SqlitePool>,
    ...
    for (index, album_row) in all_albums.into_iter().enumerate() {
        ...
        // Determine if the album should be marked as DR best
        let should_be_best = dr_store.contains(&album_key);
