...
use crate::library::file_watcher::events::{DebouncedEvent, ProcessedEvent};
            ...
            match self.event_receiver.recv().await {
                Ok(event) => {
                    ...
                    if !changed_files.is_empty() {
                        let _ = self.debounced_sender.send(DebouncedEvent::FilesChanged {
                            paths: changed_files.drain(..).collect(),
                        }).await;
                    }
                    if !removed_files.is_empty() {
                        let _ = self.debounced_sender.send(DebouncedEvent::FilesRemoved {
                            paths: removed_files.drain(..).collect(),
                        }).await;
                    }
                    if !renamed_files.is_empty() {
                        let _ = self.debounced_sender.send(DebouncedEvent::FilesRenamed {
                            paths: renamed_files.drain(..).collect(),
                        }).await;
                    }
