//! Core sample-rate converter wrapping the rubato FFT resampler.
//!
//! Pre-allocates all internal buffers so no heap allocation occurs on the
//! audio hot path. Accepts interleaved `f32` input frames and produces
//! interleaved `f32` output frames.
