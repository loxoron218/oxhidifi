...
impl Serialize for AlbumKey {
        ...
        S: Serializer,
    {
        ...
        let key_string = format!("{}<<<DRKEY>>>{}<<<DRPART>>>{}", self.title, self.artist, self.folder_path);
        ...
        D: Deserializer<'de>,
    {
        ...
        if parts.len() == 2 {
            ...
            if sub_parts.len() == 2 {
                ...
                Ok(AlbumKey { title, artist, folder_path })
            } else {
...
pub struct DrValueStore {
    ...
    impl DrValueStore {
        ...
        pub fn save(&self) -> stdResult<()> { // Specify io::Result
