...
use crate::ui::grids::artists_grid_builder::build_artists_grid;
...
/// main `ViewStack`, resetting the `artists_grid_cell` and `artists_stack_cell`,
...
/// * `artists_grid_cell` - A `Rc<RefCell<Option<FlowBox>>>` holding a reference to the `FlowBox`
...
pub fn rebuild_artists_grid_for_window(
    ...
    *artists_stack_cell.borrow_mut() = None;
    ...
    let (artists_stack, artists_grid) =
        build_artists_grid(scanning_label_artists, add_music_button);
