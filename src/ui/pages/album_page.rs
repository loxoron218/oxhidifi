...
pub async fn album_page(
    ...
    let artist: Rc<Artist> = Rc::new(
        fetch_artist_by_id(&*db_pool, album.artist_id)
            .await
            .unwrap(),
    );
    let folder: Rc<Folder> = Rc::new(
        fetch_folder_by_id(&*db_pool, album.folder_id)
            .await
            .unwrap(),
    );
    ...
    fn build_dr_badge(
        ...
        checkbox_weak.borrow().connect_toggled(move |btn| {
            ...
            MainContext::default().spawn_local(async move {
                if let Err(_e) =
                    update_album_dr_completed(&*current_db_pool, album_id, is_completed).await
                {
                }
