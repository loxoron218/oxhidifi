    ...
    drop_target.connect_drop({
        ...
        move |_target_row, value, _x, _y| {
            if let Ok(dragged_order_str) = value.get::<String>()
                && let Some(target_row) = list_row_weak.upgrade()
                && let Some(listbox) = target_row
                    .parent()
                    .and_then(|p| p.downcast::<ListBox>().ok())
            {
                ...
                for (idx, child_widget) in children_widgets.iter().enumerate() {
                    if child_widget.as_ptr() == target_row.clone().upcast_ref::<Widget>().as_ptr() {
                        target_idx = Some(idx);
                    }
                    if let Some(list_row_ref) = child_widget.downcast_ref::<ListBoxRow>()
                        && let Some(action_row_ref) = list_row_ref
                            .child()
                            .and_then(|c| c.downcast::<ActionRow>().ok())
                    {
                        let order_str_val = action_row_ref.widget_name();
                        if !order_str_val.is_empty() && order_str_val == dragged_order_str {
                            source_idx = Some(idx);
                        }
                    }
                }
                    ...
                    && from != to
                {
                    ...
                    while let Some(child_after_reorder) = current_child_after_reorder {
                        if let Some(list_row_ref) = child_after_reorder.downcast_ref::<ListBoxRow>()
                            && let Some(action_row_ref) = list_row_ref
                                .child()
                                .and_then(|c| c.downcast::<ActionRow>().ok())
                        {
                            ...
                            if !order_str_val.is_empty()
                                && let Ok(order) = SortOrder::from_str(&order_str_val)
                            {
                                new_orders.push(order);
                            }
                        }
                        current_child_after_reorder = child_after_reorder.next_sibling();
                    }
                    if !new_orders.is_empty() {
                        *sort_orders_rc.borrow_mut() = new_orders.clone();
                        ...
                        let sort_ascending_clone_drop3 = sort_ascending_clone_drop2.clone();
                        let sort_ascending_artists_clone_drop3 =
                            sort_ascending_artists_clone_drop2.clone();
                        let on_sort_changed_clone_drop2 = on_sort_changed_clone_drop.clone();
