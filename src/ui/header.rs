...
pub fn build_header_bar() -> AppHeaderBar {
    ...
    let settings_button = Button::builder().icon_name("open-menu-symbolic").build();
    ...
    let add_btn_inner = Box::builder().orientation(Orientation::Horizontal).build();
    add_btn_inner.append(&add_button);
    let add_btn_box = Clamp::builder().child(&add_btn_inner).build();
    left_btn_stack.add_titled(&add_btn_box, Some("main"), "Main");

    // Box for back button
    let back_btn_inner = Box::builder().orientation(Orientation::Horizontal).build();
    back_btn_inner.append(&back_button);
    let back_btn_box = Clamp::builder().child(&back_btn_inner).build();
...
pub fn build_tab_bar() -> (Box, ToggleButton, ToggleButton) {
    ...
    let albums_btn = ToggleButton::builder().active(true).build();
    ...
    let artists_label = Label::builder().label("Artists").build();
