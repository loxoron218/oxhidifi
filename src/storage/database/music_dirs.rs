...
impl SqliteStorage {
    ...
    pub async fn list_library_directory_rows(&self) -> Result<Vec<LibraryDirectory>, StorageError> {
    ...
    pub async fn add_library_directory_row(&self, path: &Path) -> Result<(), StorageError> {
        let path_str = path
            .to_str()
    ...
    pub async fn remove_library_directory_row(&self, id: i64) -> Result<(), StorageError> {
    ...
    pub async fn prune_orphans_row(&self) -> Result<(), StorageError> {
        _ = query(
            "DELETE FROM albums WHERE id NOT IN (SELECT DISTINCT album_id FROM tracks WHERE \
             album_id IS NOT NULL)",
        )
